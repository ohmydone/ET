...
namespace ET
{
    ...
    public static class AOITriggerSystem
    {
        ...
        float3 position1, Quaternion rotation1, float3 position2, Quaternion rotation2)
        {
            ...
            var sqrDis = math.lengthsq(pos1 - pos2);
            ...
            else if (trigger1.TriggerType <= TriggerShapeType.Cube && trigger2.TriggerType <= TriggerShapeType.Cube) //判断OBB和球触发
            {
                ...
                if (-xMax <= temp.x && temp.x <= xMax)
                {
                    ...
                    {
                        return math.lengthsq(temp - new float3(temp.x, -yMax, -zMax)) <= triggerSphere.SqrRadius;
                    }
                ...
                if (-yMax <= temp.y && temp.y <= yMax)
                {
                    ...
                    {
                        return math.lengthsq(temp - new float3(-xMax, temp.y, -zMax)) <= triggerSphere.SqrRadius;
                    }
                ...
                if (-zMax <= temp.z && temp.z <= zMax)
                {
                    ...
                    {
                        return math.lengthsq(temp - new float3(-xMax, -yMax, temp.z)) <= triggerSphere.SqrRadius;
                    }
                ...
                {
                    if (math.lengthsq(temp - (float3)points[i]) > triggerSphere.SqrRadius)
                    {
                        return false;
                    }
                }

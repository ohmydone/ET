...
namespace ET.Client
{
    [FriendOf(typeof(CircularSelectComponent))]
    public static class CircularSelectComponentSystem
    {
        ...
        {
            protected override void Update(CircularSelectComponent self)
            {
        ...
        public static bool RunCheck(this CircularSelectComponent self)
        {
            ...
            if (RaycastHelper.CastMapPoint(CameraManagerComponent.Instance.MainCamera(), out var hitPoint))
            {
                ...
                EventSystem.Instance.Publish(self.DomainScene(),new EventType.OnCircularSelect()
                {
                    pos = nowpos
                });
                return true;
